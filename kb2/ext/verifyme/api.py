...
@router.get("/email")
async def get_emails(auth_info: Annotated[UserClient, Depends(discord_auth)]):
...
async def link_email(req: LinkEmailRequest, resp: Response,
                     auth_info: Annotated[UserClient, Depends(discord_auth)]):
...
async def unlink_email(req: UnlinkEmailRequest, auth_info: Annotated[UserClient, Depends(discord_auth)]):
