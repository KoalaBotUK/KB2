...
pub trait VoteOptionComponent {
    ...
    fn custom_id(&self) -> String {
        format!("vt{}{}",self.label().clone().unwrap_or_default().replace(" ", ""), self.emoji().clone().map(|e| {
            match e {
...
pub fn group_to_rows(components: Vec<Component>) -> Result<Vec<Component>, StatusCode> {
    let rows: Vec<Component> = components.chunks(5).map(|chunk| Component::ActionRow(ActionRow { components: Vec::from(chunk)})).collect();
    if rows.len() > 5 {
